...
const UserDashboard = () => {
  ...
  const handleJoinRequest = async (groupId, groupName) => {
    ...
    try {
      ...
      const alreadyPending = pendingRequests.some(req => req.groupId === groupId);
      if (alreadyPending) {
        setMessage(groupId, "error", "You already have a pending request for this group");
        return;
      }
      ...
      setMessage(groupId, "success", "Join request sent to admin! You'll be notified when approved.");
      ...
      setMessage(groupId, "error", err.message || "Failed to send request. Please try again.");
    } finally {
      ...
      <div className="dashboard-page">
        <div className="dashboard-inner">
          ...
          <div className="section-card">
            ...
            {loadingGroups ? (
              <p style={{ color: 'var(--text-muted)', padding: '1rem 0' }}>Loading groups...</p>
            ) : groups.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', padding: '1rem 0' }}>No groups available to join.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {groups.map(group => {
                  const msg = messageMap[group.id];
                  const isRequesting = requestingGroupId === group.id;
                  const isFull = (group.members?.length || 0) >= (group.maxMembers || Infinity);
                  const isMember = userGroups.includes(group.id);
                  const hasPendingRequest = pendingRequests.some(req => req.groupId === group.id);
                  
                  return (
                    <div key={group.id} className="group-row" style={{
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'space-between',
                      ...
                      {/* Group Details */}
                      <div style={{ flex: 1, minWidth: '150px', display: 'flex', gap: '1.5rem', fontSize: '0.875rem' }}>
                        {group.contributionAmount && (
                          <div>
                            <div style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Monthly</div>
                      ...
                      <div style={{ minWidth: '160px' }}>
                        {isMember ? (
                          ...
                          <button 
                            ...
                            style={{ width: '100%' }}
                            onClick={() => handleJoinRequest(group.id, group.groupName || group.name)}
      ...
      <div className="dashboard-inner">
        ...
        <div className="stats-grid" style={{ marginBottom: '2rem' }}>
          ...
          <div className="stat-card accent-blue">
            ...
            <div className="stat-value">{new Date(nextPayment).toLocaleDateString()}</div>
            <div className="stat-sub">amount: R{upcomingPayments[0]?.amount}</div>
          </div>
